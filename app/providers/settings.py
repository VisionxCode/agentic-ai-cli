...
APP_HOME_ENV = "IBM_HACKATHON_AGENT_HOME"
DEFAULT_APP_HOME = ".ibm_hackathon_agent"
CONFIG_FILENAME = "config.json"


def app_home() -> Path:
    configured = os.getenv(APP_HOME_ENV)
    if configured:
...
def user_config_path() -> Path:
    configured = os.getenv("IBM_HACKATHON_AGENT_CONFIG")
    if configured:
        return Path(configured).expanduser()
...
def read_user_config() -> dict[str, Any]:
    if os.environ.get("PYTEST_CURRENT_TEST") and not (
        os.getenv(APP_HOME_ENV) or os.getenv("IBM_HACKATHON_AGENT_CONFIG")
    ):
        return {}
